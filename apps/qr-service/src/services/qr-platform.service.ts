...
export class QrPlatformService {
  ...
  async createExternalQrCode(dto: ApiCreateQrDto, raw?: string, idempotencyKey?: string) {
    ...
    const replay = await this.replayIdempotent(key, "qr.create", dto, idempotencyKey);
    if (replay !== undefined) return replay;
    const consumed = await this.consumeRentalQuota(key, 1);
    try {
      return await this.transaction(key, "qr.create", dto, idempotencyKey, async (tx, current) => this.createOne(tx, current, dto));
    } catch (error) {
      if (consumed) await this.refundRentalQuota(key, 1);
      throw error;
    }
  }
  ...
  async createBulk(resources: ApiCreateQrDto[], raw?: string, idempotencyKey?: string) {
    ...
    const replay = await this.replayIdempotent(key, "qr.bulk", { resources }, idempotencyKey);
    if (replay !== undefined) return replay;
    const consumed = await this.consumeRentalQuota(key, resources.length);
    try {
      return await this.transaction(key, "qr.bulk", { resources }, idempotencyKey, async (tx, current) => {
        const results = [];
      ...
      if (consumed) await this.refundRentalQuota(key, resources.length);
      throw error;
    }
  }
  ...
  private async replayIdempotent<T>(key: IntegrationKey, operation: string, body: unknown, idempotencyKey: string | undefined): Promise<T | undefined> {
    if (idempotencyKey === undefined) return undefined;
    if (!/^[\x21-\x7E]{1,200}$/.test(idempotencyKey)) {
      throw new BadRequestException({ error: "invalid_idempotency_key", message: "Idempotency-Key must contain 1-200 printable characters without spaces" });
    }
    const scopeId = `${resolveTenantId(key)}:${key.isTest ? "test" : "live"}`;
    const prior = await this.prisma.apiIdempotency.findUnique({
      where: { scopeId_operation_key: { scopeId, operation, key: idempotencyKey } }
    });
    if (!prior || prior.expiresAt <= new Date()) return undefined;
    const requestHash = crypto.createHash("sha256").update(this.canonical(body)).digest("hex");
    if (prior.requestHash !== requestHash) {
      throw new ConflictException({ error: "idempotency_conflict", message: "Idempotency-Key was already used with a different request" });
    }
    return prior.response as T;
  }

  private async consumeRentalQuota(key: IntegrationKey, amount: number) {
    if (key.isTest || !key.rentalId) return false;
    return this.rentalQuotaRequest(key.rentalId, "consume-quota", amount);
  }

  private async refundRentalQuota(key: IntegrationKey, amount: number) {
    if (key.isTest || !key.rentalId) return;
    await this.rentalQuotaRequest(key.rentalId, "refund-quota", amount);
  }

  private async rentalQuotaRequest(rentalId: string, action: "consume-quota" | "refund-quota", amount: number) {
    const base = process.env.RENTAL_SERVICE_URL ?? "http://localhost:3004";
    const token = process.env.INTERNAL_SERVICE_TOKEN;
    if (!token) throw new ServiceUnavailableException({ error: "rental_service_unavailable", message: "INTERNAL_SERVICE_TOKEN is not configured" });
    let response: Awaited<ReturnType<typeof fetch>>;
    try {
      response = await fetch(`${base}/internal/rentals/${encodeURIComponent(rentalId)}/${action}`, {
        method: "POST",
      ...
      throw new ServiceUnavailableException({ error: "rental_service_unavailable", message: "Rental service is unavailable" }, { cause: error instanceof Error ? error : undefined });
    }
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      if (response.status === 409 && payload?.error === "quota_exceeded") {
        throw new ConflictException({ error: "quota_exceeded", message: payload.message ?? "Rental has reached its quota limit" });
      }
      throw new ServiceUnavailableException({ error: "rental_service_unavailable", message: payload?.message ?? "Rental service request failed" });
    }
    return true;
  }
